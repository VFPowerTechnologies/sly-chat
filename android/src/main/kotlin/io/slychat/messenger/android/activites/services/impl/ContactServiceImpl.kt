...
class ContactServiceImpl(private val activity: AppCompatActivity): ContactService {
    ...
    override fun fetchNewContactInfo(username: String): Promise<ContactInfo?, Exception> {
        val email = parseEmail(username)
    ...
    private fun parseEmail(username: String): String? {
        ...
        return if (ptr.matcher(username).matches())
            username
        else
            null
    }

    private fun parsePhoneNumber(username: String): String? {
        val utils = PhoneNumberUtil.getInstance()
        val defaultRegion: String
        val accountInfo = app.accountInfo

        if (accountInfo !== null)
            defaultRegion = getAccountRegionCode(accountInfo)
        else
            defaultRegion = activity.resources.configuration.locale.country


        try {
            val phoneNumber: Phonenumber.PhoneNumber
            if (username.startsWith("+")) {
                phoneNumber = utils.parse(username, null)
            }
            else {
                phoneNumber = utils.parse(username, defaultRegion)
            }

            return utils.format(phoneNumber, PhoneNumberUtil.PhoneNumberFormat.E164).replace("+", "")
        }
